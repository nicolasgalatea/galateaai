...
import { Upload, Loader2, CheckCircle, BookOpen } from 'lucide-react';
...
export default function AgentLogbook() {
  ...
  const handleAnalyze = async () => {
    ...
    setResults({
      diagnosis: 'Surgical logbook entry created and organized',
      confidence: 94,
      recommendations: [
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
            ...
            <div className="flex flex-col justify-center">
              <h1 className="text-4xl md:text-5xl font-bold mb-4">
                {t('agents.logbook.name')}
              </h1>
              <div className="inline-block px-4 py-2 bg-primary/10 text-primary rounded-full text-sm font-medium mb-6 w-fit">
                {t('agents.logbook.specialty')}
              </div>
              <p className="text-lg text-muted-foreground mb-6">
                {t('agents.logbook.desc')}
              </p>
              <p className="text-muted-foreground">
                Organizes procedure logs, dates, surgical techniques, complications, and surgeon case tracking. Essential for surgical training and performance monitoring.
              </p>
            </div>
          </div>
